...
FastRender._securityCheck = function(payload) {
  ...
  if (Meteor.settings && Meteor.settings.public && Meteor.settings.public.fastRenderDisableSecurityHosts) {
    hosts = Meteor.settings.public.fastRenderDisableSecurityHosts
    hosts.push('http://localhost')
  }
  var url = Meteor.absoluteUrl();
  for (var i = 0; i < hosts.length; i++) {
    var loc = hosts[i];
    if (url.substr(0, loc.length) == loc)
      return true;
  }
